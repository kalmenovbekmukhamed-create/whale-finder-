...
with st.expander("❔ Как это работает · честная методология"):
    st.markdown("""
**Что это.** Whale Finder следит за **XAUT/USD** (Tether Gold) — токеном, который
повторяет реальную цену золота. Данные — настоящие свечи и объёмы с биржи **Bitfinex**.

**Кто такой «кит».** Свеча, чей реальный объём одновременно (1)&nbsp;**≥&nbsp;2.5×
скользящей медианы** и (2)&nbsp;статистически необычен — **z-оценка ≥&nbsp;2.5**
(медиана&nbsp;+&nbsp;MAD). Оба порога меняются в **⚙&nbsp;Чувствительность**.

**Покупает или продаёт.** По положению закрытия внутри свечи (Close Location Value):
закрытие у максимума → **давление вверх**, у минимума → **давление вниз**.

**🔥 Карта.** Реальный объём, распределённый по ценам во времени — горячие
горизонтальные полосы показывают уровни, где торговали активнее всего. Это
**объём, а не биржевой стакан**.
...
Это **описательная статистика на истории — не прогноз** и не гарантия прибыли.

**Честно о данных.** Это OHLCV одной биржи — **не** поток ордеров и **не** биржевой
стакан. Ни одна цифра не выдумана: всё считается из реальных данных. — *Bit&#95;Generation*
""")
